...
export const SideNavbar : FunctionComponent<SideNavBarProps> = ({selected_page, apptheme, generalcs, changePg, createNote}) => {
    ...
    return (
        <div className="w-[40px] h-[179px]">
            <motion.div className="w-[40px] h-[40px] rounded-full flex items-center justify-center" style={{background: generalcs}}
                whileHover={{scale: 1.03}} whileTap={{scale: 0.97}} onClick={() => set_createNewNoteDialogue(!show_createNewNoteDialogue)}
                >
                <Plusicon />
            </motion.div>
